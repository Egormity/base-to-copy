...
class LibraryFoldersServiceClass {
  ...
  public async getLibraryFolders({
    ...
    try {
      const response = await axiosWithAuth.get<ILibraryFoldersResponse>(this.BASE_URL, {
        params: {
          search: searchQuery,
          id,
          folderTypeId,
  ...
  public async createLibraryFolder(data: any): Promise<ILibraryFoldersResponse> {
    // console.log(data);
    try {
      const response = await axiosWithAuth.post<ILibraryFoldersResponse>(this.BASE_URL, data);
  ...
  public async updateLibraryFolder({
    ...
    try {
      const response = await axiosWithAuth.put<ILibraryFoldersResponse>(`${this.BASE_URL}/${id}`, data);
      return response.data;
    } catch (error) {
      console.log(error);
