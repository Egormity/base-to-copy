...
import { useGetOrganizationTypes, useGetProducts, useGetSpecializations } from '@/api/commons';
...
const FilterModal: FC<IFilterModal> = () => {
  ...
  const { items: products1, isLoading: isLoadingProducts1 } = useGetProducts(searchBrand);
  const brands = products1?.map(product => ({
    id: product.id,
    name: product.name,
  }));

  const { items: products2, isLoading: isLoadingProducts2 } = useGetProducts(searchDrugs);
  ...
  const { applyFilters, clearFilters, filters, addFilter, removeFilter } = useFilterLibraryStore();
