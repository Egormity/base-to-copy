...
const FilterTargetItem: FC<IFilterTargetItem> = ({
  ...
  const targetFilterIds = filters.find(filter => filter.name === 'targetUserIds');

  const appliedTargetFilter = appliedFilters.find(filter => filter.name === 'isTarget');

  const [isToggled, setIsToggled] = useState(appliedTargetFilter?.values[0] || targetFilterIds?.values);
  ...
  const handleRemoveTarget = (id: string) => {
    const updatedTargets = targetFilterIds?.values?.filter(
      (target: { id: string; name: string; postName: string }) => target.id !== id
    );
      ...
      {isToggled && (
        <>
          ...
          <ul className='flex flex-col gap-2'>
            {targetFilterIds?.values?.map((target: { id: string; name: string; postName: string }) => (
              <li key={target.id} className='p-2 border-primary-gray-light border-[1px] rounded-lg'>
                <div className='flex flex-row justify-between'>
                  <div className='flex flex-col gap-2'>
                    <div className='text-sm'>{target.name}</div>
                    <div className='text-[#9B9B9B] text-[10px]'>{target.postName}</div>
                  </div>
                  <div className='cursor-pointer' onClick={() => handleRemoveTarget(target.id)}>
