import { type FC, type InputHTMLAttributes, type ReactNode, useState } from 'react';
...
export const InputFile: FC<IInput> = ({
  ...
  return (
    <div
      className={`${bigInput ? styles['input-wrapper-big'] : styles['input-wrapper']} ${
        classNameWrapper ?? ''
      }`}
    >
      {label && (
        <div
          className={`${bigInput ? styles['input-title-big'] : styles['input-title']} ${
            classNameTitle ?? ''
          }`}
        >
      ...
      <label
        className={`${bigInput ? styles['container-big'] : styles['container']} ${
          classNameContainer ?? ''
      ...
      >
        <div className='flex items-center gap-2'>
          {inputValue && !bigInput ? null : (
            <span className='text-sm h-full w-full text-primary-gray-dark'>
        ...
        <input
          onChange={event => {
            ...
            if (!isValidFileType(fileName)) {
              onInvalidFileType && onInvalidFileType(`Неправильный тип файла: ${fileName}`);
              return;
            }
