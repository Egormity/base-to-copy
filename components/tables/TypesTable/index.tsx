...
const TypesTable = <TData extends object>({
  ...
  return (
    <>
      <div className={cn('h-full max-h-[calc(100vh-60px)] overflow-auto scrollbar-thin', className)}>
        <table className='w-full h-full border-collapse'>
          <thead>
            {table.getHeaderGroups().map(headerGroup => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map(header => (
                  <th key={header.id} className='text-left py-3 px-5 border-r border-gray-300 text-[16px]'>
          ...
          <tbody>
            {table.getRowModel().rows.map(row => (
              <tr key={row.id} className='border-y border-gray-300'>
                        ...
                        cell
                          ...
                          .map((item, index) =>
                            typeof item === 'object' && item.name ? (
                              <div className='text-sm font-light' key={index}>
                                {item.name}
                              </div>
                            ) : (
            ...
            <tr>
              {columns.map((_, index) => (
                <td key={index} className='flex-1 text-center h-full p-5 border-r border-gray-300'></td>
              ))}
            </tr>
          </tbody>
        </table>
