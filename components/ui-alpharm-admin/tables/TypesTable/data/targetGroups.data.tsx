...
import { ITargetGroup } from "@/services/dictionaries/target-groups";
...
export const defaultColumns: ColumnDef<ITargetGroup, any>[] = [
	columnHelper.accessor("name", {
		header: () => (
			<span className="text-sm font-normal">Название целевой группы</span>
		),
		cell: (info) => (
	...
	columnHelper.accessor((row) => row.targetGroupTypeName, {
		...
		header: () => <span className="text-sm font-normal">Тип группы</span>,
		footer: (info) => info.column.id,
	}),
	columnHelper.accessor("frequency", {
		header: () => <span className="text-sm font-normal">Кратность</span>,
		cell: (info) => (
